...
const createUpdateIdentifierPersona =
  (config: Config) =>
  async ({
    organisation,
    ifi,
    ...
    // find the ifi
    try {
      // Identifier exists, so do nothing.
      ...
      if (err instanceof ExpiredLock) {
        const { identifier: expiredIdentifier, ignorePersonaId } = err;
...
const retryCreateUpdateIdentifierPersona =
  (config: Config) =>
  async (opts: TheCreateUpdateIdentifierPersonaOptions): Promise<CreateUpdateIdentifierPersonaResult> => {
