...
import { PERSONA_ATTRIBUTES_COLLECTION, PERSONA_IDENTIFIERS_COLLECTION } from './utils/constants/collections';
...
export default (config: Config) => {
  return async (opts: MergePersonaOptions): Promise<MergePersonaResult> => {
    ...
    const toMaybeUpdateIdentifierIds = (await personaIdentifiersCollection.find(identFilter).toArray()).map(({ _id }) =>
      _id.toString()
    );
    ...
    const toMaybeUpdateAttributeIds = (await personaAttributesCollection.find(attributeFilter).toArray()).map(
      ({ _id }) => _id.toString()
    );
