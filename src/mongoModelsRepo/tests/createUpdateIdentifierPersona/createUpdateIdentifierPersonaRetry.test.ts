...
describe('createUpdateIdentifierPersona mongo retry', () => {
  ...
  const getMongoDB = async () => {
    const client = await MongoClient.connect(config.mongoModelsRepo.url, config.mongoModelsRepo.options);
  ...
  it('should error if unlocked, but persona is not set, (should not be possible in rl)', async () => {
    const repoConfig = { db: getMongoDB() };
