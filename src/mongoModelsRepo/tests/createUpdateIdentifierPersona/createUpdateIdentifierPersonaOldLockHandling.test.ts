...
describe('createUpdateIdentifierPersona identifier old lock handling mongo', () => {
  ...
  const generateMockDb = async () =>
    (await MongoClient.connect(config.mongoModelsRepo.url, config.mongoModelsRepo.options)).db();
  ...
  it('Should handle a locked personaIdentifier without a lockedAt field', async () => {
    ...
    const { identifier: correctedIdentifier, wasCreated } = await createUpdateIdentifierPersona(dbConfig)(actor);
    ...
    assert.notEqual(
      correctedIdentifier.persona,
      testIdentifier.persona,
      'Persona should have been replaced with a new one'
    );
    ...
    const identifierDocument = await (
      await getPersonaIdentifiersCollection(dbConfig)
