...
describe('', () => {
  ...
  const generateMockDb = async () => {
    return (await MongoClient.connect(config.mongoModelsRepo.url, config.mongoModelsRepo.options)).db();
  };

  it("Should create a new persona and add toidentifier if identifier doens't have one", async () => {
    ...
    const { identifier: identifierWithPersona, wasCreated } = await createPersonaAndAddToIdentifier(dbConfig)({
  ...
  it('Should just return identifier unchanged if it already has persona', async () => {
    ...
    const { identifier: unchangedIdentifier, wasCreated } = await createPersonaAndAddToIdentifier(dbConfig)({
