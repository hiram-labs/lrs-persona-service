...
export default (config: Config, collectionName: string) => {
  return async <T extends BaseModel>({
    ...
    const filters = [filter, cursorFilter].filter((item) => !isUndefined(item) && !isEmpty(item));

    const combinedFilters = filters.length <= 1 ? filters[0] : { $and: filters };
    ...
    const result2: PaginationResult<T> = {
      ...
      pageInfo: {
        ...
        hasNextPage:
          (direction === CursorDirection.BACKWARDS && cursor !== undefined) ||
          (direction === CursorDirection.FORWARDS && result.length > limit),
        hasPreviousPage:
          (direction === CursorDirection.FORWARDS && cursor !== undefined) ||
