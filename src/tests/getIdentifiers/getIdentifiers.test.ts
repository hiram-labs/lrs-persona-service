...
describe('getIdentifiers', () => {
  ...
  const getMboxFromPersona = (persona: Persona, index: number) =>
    `${persona.name?.replace(/\s/g, '_')}_${index}@test.com`;

  const addTestIdentifiers = async (customPersona?: Persona, amount = 12): Promise<Identifier[]> => {
    const persona = customPersona === undefined ? await createTestPersona() : customPersona;
    const results = [];
    ...
    for (let i = 0; i < amount; i++) {
      results.push(
        await service.createIdentifier({
          ifi: {
            key: 'mbox',
            value: customPersona === undefined ? `${i}_${TEST_IFI.value}` : getMboxFromPersona(persona, i)
          },
          organisation: TEST_ORGANISATION,
          persona: persona.id
        })
  ...
  it('Should return the first 10 items', async () => {
    ...
    assert.equal(
      identifiersResults.edges[0].cursor,
      modelToCursor({
        model: {
          ifi: {
  ...
  it('should properly use combination of filter with $or and cursor', async () => {
    ...
    assert.equal(
      ...
      [getMboxFromPersona(secondPersona, 1)].join(',')
    );
  });
});
