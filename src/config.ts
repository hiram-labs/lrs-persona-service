...
const getMongoReadPreference = (readPrefMode = 'primary'): ReadPreference => {
  const mode = (ReadPreference.isValid(readPrefMode) ? readPrefMode : 'primary') as ReadPreferenceMode;
...
export default {
  ...
  winston: {
    cloudWatch: {
      ...
      logGroupName: getStringOption(process.env.WINSTON_CLOUDWATCH_LOG_GROUP_NAME, 'persona-service'),
